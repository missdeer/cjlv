...
{
    auto it = std::find_if(m_extensions.begin(), m_extensions.end(), [=](ExtensionPtr e) { return e->uuid() == uuid; });
...
{
    auto it = std::find_if(m_extensions.begin(), m_extensions.end(), [e](ExtensionPtr ie) { return ie->uuid() == e->uuid(); });
