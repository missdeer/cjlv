...
BOOL CShellContextMenu::GetContextMenu (void ** ppContextMenu, int & iMenuType)
{
	*ppContextMenu = NULL;
	...
	m_psfFolder->GetUIObjectOf (NULL, (UINT)nItems, (LPCITEMIDLIST *) m_pidlArray, IID_IContextMenu, NULL, (void**) &icm1);
...
UINT CShellContextMenu::ShowContextMenu(QMenu *pWnd, const QPoint &pt)
{
    ...
    HMENU hMenu = (HMENU)m_Menu->winId();
	...
	{
        OldWndProc = (WNDPROC) SetWindowLong ((HWND)pWnd->winId(), GWLP_WNDPROC, (LONG)HookWndProc);
		if (iMenuType == 2)
	...
	if (OldWndProc) // unsubclass
        SetWindowLong ((HWND)pWnd->winId(), GWLP_WNDPROC, (LONG) OldWndProc);
...
void CShellContextMenu::SetObjects(const QStringList &strArray)
{
	// free all allocated datas
	if (m_psfFolder && bDelete)
		m_psfFolder->Release ();
    ...
    QString s = strArray.at(0);
    wchar_t ws[MAX_PATH] = {0};
    wcscpy(ws, s.toStdWString().c_str());
    psfDesktop->ParseDisplayName (NULL, 0, ws, NULL, &pidl, NULL);
    ...
    nItems = strArray.length();
	for (int i = 0; i < nItems; i++)
    {
        QString s = strArray.at (i);
        wchar_t ws[MAX_PATH] = {0};
        wcscpy(ws, s.toStdWString().c_str());
        psfDesktop->ParseDisplayName (NULL, 0, ws, NULL, &pidl, NULL);
...
QMenu *CShellContextMenu::GetMenu()
{
	if (!m_Menu)
	{
        m_Menu = new QMenu;
